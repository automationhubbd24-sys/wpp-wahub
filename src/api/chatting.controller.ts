...
import { BanProtectionService } from '../core/services/BanProtectionService';
import { MessageQueueService } from '../core/queue/MessageQueueService';
...
export class ChattingController {
  ...
  async sendText(@Body() request: MessageTextRequest): Promise<WAMessage> {
    ...
    // 2. Add invisible variation & Emoji randomization to bypass AI fingerprinting
    request.text = this.banProtection.applyInvisibleVariation(request.text);
    request.text = this.banProtection.randomizeEmojis(request.text);

    // 3. Simulate human behavior (Typing, Random Delays)
  ...
  @Post('/sendTextBalanced')
  @ApiOperation({ 
    summary: 'Send a text message across all working sessions (Load Balanced)',
    description: 'Picks a random working session to send the message, reducing the risk of bans.'
  })
  async sendTextBalanced(@Body() request: MessageTextRequest): Promise<WAMessage> {
    const allSessions = await this.manager.getSessions(false);
    const workingSessions = allSessions.filter(s => s.status === WAHASessionStatus.WORKING);

    if (workingSessions.length === 0) {
      throw new UnprocessableEntityException('No working sessions available for load balancing.');
    }

    // Pick a random working session
    const randomSession = workingSessions[Math.floor(Math.random() * workingSessions.length)];
    request.session = randomSession.name;
  ...
  @Post('/sendTextQueued')
  @ApiOperation({ 
    summary: 'Send a text message via a Smart Queue (Paced)',
